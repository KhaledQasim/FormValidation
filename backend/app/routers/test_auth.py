...
def test_create_duplicate_user_username():
    ...
    response = client.post(
        "/auth",
        json={"email": "test2@test.com", "password": "12345Test?","username": "test"},
    )
...
def test_create_duplicate_user_email():
    ...
    response = client.post(
        "/auth",
        json={"email": "test@test.com", "password": "12345Test?","username": "test1"},
    )
    assert response.is_error
...
def test_create_user_with_wrong_username_syntax():
    ...
    username_too_short_response = client.post(
        "/auth",
        json={"email": "test1@test.com", "password": "12345Test?","username": "te"},
    )
    ...
    username_too_long_response = client.post(
        "/auth",
        json={"email": "test2@test.com", "password": "12345Test?","username": "test5678900000"},
    )
...
def test_create_user_with_wrong_email_syntax():
    ...
    no_at_email_symbol_response.is_error
    
    
    no_dot_response = client.post(
        "/auth",
        json={"email": "test@testcom", "password": "12345Test?","username": "test1"},
    )
    no_dot_response.is_error

    #email is 31 characters long
    email_too_long_response = client.post(
        "/auth",
        json={"email": "test@test.cooooooooooooooooooom", "password": "12345Test?","username": "test2"},
    )
    email_too_long_response.is_error
    ...
    email_too_short_response.is_error
...
def create_user_with_wrong_password_syntax():
   
    password_too_short = client.post(
        "/auth",
        json={"email": "test@test.com", "password": "1Test?","username": "test"},
    )
    assert password_too_short.status_code == 200
    
    password_too_long = client.post(
        "/auth",
        json={"email": "test1@test.com", "password": "12345Test?321231231321314124124","username":"test1"},
    )
    ...
    password_with_no_upperCase= client.post(
        "/auth",
        json={"email": "test3@test.com", "password": "12345test?", "username":"test3"},
    )
    assert password_with_no_upperCase.is_error
    
    password_with_no_number= client.post(
        "/auth",
        json={"email": "test4@test.com", "password": "TestTestTest?","username":"test4"},
    )
    assert password_with_no_number.is_error
    
    password_with_no_special= client.post(
        "/auth",
        data={"email": "test5@test.com", "password": "TestTestTest1","username":"test5"},
    )
